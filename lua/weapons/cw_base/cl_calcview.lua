...
function SWEP:CalcView(ply, pos, ang, fov)
	...
	if self.dt.State == CW_AIMING then
		...
		self.CurFOVMod = Lerp(FT * 5, self.CurFOVMod, 0)
			...
			if self.dt.State == CW20_DEPLOY then
		self.CurFOVMod = self.CurFOVMod
		else
		self.CurFOVMod = Lerp(FT * 5, self.CurFOVMod, 0)
		end
	...
	if self.Owner then
		if self.ViewbobEnabled then
			...
			velorig = Length(GetVelocity(self.Owner))
			vel = math.Clamp(velorig, 0, 500)
			
			if self.Owner:OnGround() and vel > ws * 0.1 then			
					mod = 8.55
					cos1 = vel * ((math.cos(CT * (16))) * 0.005)
					cos2 = vel * (math.cos(CT * 8)) * 0.01
					cos3 = vel * (math.sin(CT * 8)) * 0.005
					curviewbob.p = vel * (cos1 * 0.4) * 0.001
					curviewbob.y = vel * (cos2 * 0.5) * 0.001
					curviewbob.z = vel * (cos3 * -0.7) * 0.001
			else
				curviewbob = LerpAngle(FT * 10, curviewbob, Ang0)		
					mod = 0
					cos1 = vel / 500
					curviewbob.p = vel * (mod * -0.7) * 0.005
			end
		end
	end
...
function SWEP.CreateMove(move)
	...
	if IsValid(wep) and wep.CW20Weapon then
		...
		if wep.dt and wep.dt.State == CW_AIMING then
			...
			ang.p = ang.p - math.cos(CT * 1.3) ^ 5 * math.tan(-0.03, 100) * math.sin(1/2) * wep.CurBreatheIntensity * 0.1
			ang.y = ang.y + math.sin(CT * 0.8 * math.cos(-0.2, 1000)) * 0.001 * wep.AimBreathingIntensity * wep.CurBreatheIntensity * 0.05
			else
			ang.p = ang.p - math.cos(CT * 1.3) * math.tan(-0.02, 0.02) * math.sin(1/2) * wep.CurBreatheIntensity * 0.5
