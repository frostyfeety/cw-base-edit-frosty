...
function SWEP:IndividualInitialize()
	self.CWCEmptySound = true
	if self.MuzzleVelocity < 300 then
		self.MuzzleVelocity = 300
	end
end
...
function SWEP:Holster(wep)
	...
	if self:filterPrediction() then
		if self.holsterSound then -- quick'n'dirty prediction fix
			self:EmitSound("CWC_FOCUS", 70, 100)
...
function SWEP:simulateRecoil()
	if  self.dt.BipodDeployed then
		self.FireMove = math.Clamp(self.Recoil * self.FireMoveMod, 1, 1)
	else
		if self.dt.State == CW_AIMING then
			if self.Owner:KeyDown(IN_SPEED) then
				self.FireMove = math.Clamp(self.Recoil * self.FireMoveMod * 1.5, 1, 1.8)
			else
				self.FireMove = math.Clamp(self.Recoil * self.FireMoveMod * 2, 1, 1.8)
			end
		else
		self.FireMove = 0.4
		end
	...
	if GetConVarNumber("cwc_fovpunch") > 0 then
		if self.dt.State ~= CW_AIMING then		
			self.FOVHoldTime = UnPredictedCurTime()
		else		
			if self.FireMode == "auto" then
				self.FOVHoldTime = UnPredictedCurTime() + self.FireDelay * 0.2
			else
				self.FOVHoldTime = UnPredictedCurTime()
			end
		end
		...
		if self.HipFireFOVIncrease then
			if self.dt.State ~= CW_AIMING then		
				self.FOVTarget = -math.Clamp(self.FOVTarget + 3 + (self.Damage * 0.3) * self.FOVPerShot, 0, 2)
			else		
				self.FOVTarget = math.Clamp(self.FOVTarget + 1.2 + (self.Recoil * 0.2) + (self.MaxSpreadInc * 3) + (self.FireDelay * 70) - 8 * self.FOVPerShot, 0, 7)
			end
		end
	end
...
function SWEP:postPrimaryAttack()
		if self.FireMode == "semi" and self.Owner:KeyPressed(IN_ATTACK) and self.FireDelay < 0.12 then
	self:delayEverything(self.FireDelay + 0.05)
	end
end
