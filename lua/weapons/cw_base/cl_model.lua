...
function SWEP:performViewmodelMovement()
	...
	veldepend.roll = math.Clamp((vel:DotProduct(EA:Right()) * 0.07) * len / ws, -7, 7)
	...
	elseif self.dt.State == CW_ACTION or self.dt.State == CW_HOLSTER_START or self.dt.State == CW_HOLSTER_END then
		...
		TargetPos, TargetAng = self.SwimPos * 2, self.SwimAng * 0.5
	...
	elseif self.dt.State == CW_RUNNING or (((len > ws * self.RunStateVelocity and self.Owner:KeyDown(IN_SPEED)) or len > ws * 3 or (self.ForceRunStateVelocity and len > self.ForceRunStateVelocity)) and self.Owner:OnGround()) then
		...
		if not self.DisableSprintViewSimulation then
			...
			TargetAng.x = TargetAng.x - math.Clamp(verticalOffset, 0, 0) * self.SprintViewNormals.x
			TargetAng.y = TargetAng.y - verticalOffset * 0.0 * self.SprintViewNormals.y
			TargetAng.z = TargetAng.z - verticalOffset * 0.0 * self.SprintViewNormals.z
			TargetPos.z = TargetPos.z + math.Clamp(verticalOffset * -0.2,verticalOffset * -0.2, 2)
		end
	...
	elseif self.dt.State == CW_PRONE_BUSY then
		...
		AngMod.x = AngMod.x + tan1 * 1 * self.ViewModelMovementScale * mul
		AngMod.y = AngMod.y - cos1 * -3 * self.ViewModelMovementScale * mul
	...
	if len < 10 or not self.Owner:OnGround() then
		...
		if self.dt.State != CW_AIMING and not self:isBipodIdle() then
			...
			tan2 = math.sin(cos1 + sin1, cos1 / sin1)
			-- flick = math.sin((sin1 * 4) * tan2 ^ 6)
			
			if self.Owner:Crouching() then
				if not self.Owner:OnGround() then
				AngMod.z = AngMod.z + cos1 -10
				AngMod.x = AngMod.x + tan - 0
				AngMod.y = AngMod.y + cos1  - 0
				PosMod.y = PosMod.y - 2
				PosMod.x = PosMod.x + 1
				PosMod.z = PosMod.z - 0.5
				else
				AngMod.z = AngMod.z + cos1 -0
				AngMod.x = AngMod.x + tan - 0
				AngMod.y = AngMod.y + cos1  - 0
				PosMod.y = PosMod.y + 1
				PosMod.x = PosMod.x - 0.2
				PosMod.z = PosMod.z + 0.5
				end
			elseif not self.Owner:OnGround() then
			AngMod.z = AngMod.z - 5 + cos1 - sin1 * -8
			AngMod.x = AngMod.x + 2
			AngMod.y = AngMod.y + cos1 * 2
			PosMod.y = PosMod.y - 1.5
			PosMod.x = PosMod.x - 0
			PosMod.z = PosMod.z - 0.5
			else
			AngMod.z = AngMod.z + cos1 - sin1 * 0.5
			AngMod.x = AngMod.x + tan / sin1 * 0.82
			AngMod.y = AngMod.y + cos1 * tan * -0.3
				if self.Owner:KeyPressed(IN_ATTACK) then
					PosMod.y = PosMod.y + 0
					PosMod.x = PosMod.x + 0
					PosMod.z = PosMod.z + 0
				else
					PosMod.y = PosMod.y - sin1 * tan / cos1 * tan2 * -0.1
					-- PosMod.y = PosMod.y + flick * 1
					PosMod.x = PosMod.x - cos1 * -0.1
					PosMod.z = PosMod.z - cos1 * sin1 * 0.0
				end
			end
			
			
			PosMod.y = PosMod.y - tan * 0.5 * mod2 / cos1
		end
	elseif len > 10 and len < ws * 1.2 then
		...
		sin1 = math.sin(CT * mod) * mul * -1.5
		cos1 = math.cos(CT * mod) * mul * 0.5
		tan1 = math.atan(cos1 * sin1, cos1 * sin1) * mul * -1
		alt = math.tan(tan1 ^ 5 * 45 - 25) * mul
		...
		if self.ViewModelFlip then
			...
			if self.Owner:Crouching() and self.dt.State ~= CW_AIMING then
		AngMod.z = AngMod.z + cos1 -5 + self:scaleMovement(sin1 * -5, mod2)
		AngMod.x = AngMod.x + sin1 - 0 + self:scaleMovement(tan1 * 2, mod2)
		AngMod.y = AngMod.y + cos1  - 0 + self:scaleMovement(cos1 * -3.5, mod2)
		PosMod.y = PosMod.y + 1 + self:scaleMovement(sin1 * 2, mod2)
		PosMod.x = PosMod.x - 0.2 + self:scaleMovement(sin1 * 0.18, mod2)
		PosMod.z = PosMod.z + 0.5
		else
				if len < ws * 0.6 then
					if self.dt.State ~= CW_AIMING then
						AngMod.x = AngMod.x + self:scaleMovement(-sin1 ^ 8, alt + 3.0) * 2.5 -- up/down
					else
						AngMod.x = AngMod.x + self:scaleMovement(-sin1 ^ 8, alt + 3.0) * 0.5 -- up/down
						PosMod.x = PosMod.x * self:scaleMovement(sin1 * 3.1, mod2) -- left/right
					end
					AngMod.z = AngMod.z + self:scaleMovement(tan1 * 16, mod2) -- rotation left/right
					AngMod.y = AngMod.y + self:scaleMovement(sin1 * 2.4, 0.5) -- left/right
					PosMod.x = PosMod.x * self:scaleMovement(tan1 * 1.1, mod2) -- left/right
				else
					AngMod.x = AngMod.x + self:scaleMovement(-sin1 ^ 6, alt + 1.2) / 20.5 -- up/down
					AngMod.z = AngMod.z + self:scaleMovement(tan1 * 2, mod2) -- rotation left/right
					AngMod.y = AngMod.y + self:scaleMovement(sin1 * 2.5, 0.3) -- left/right
					PosMod.x = PosMod.x * self:scaleMovement(sin1 * 3.1, mod2) -- left/right
				end
			if self.dt.State ~= CW_AIMING then
				-- PosMod.z = PosMod.z + self:scaleMovement(-sin1 ^ 4, alt + 0.0001) * -0.02
				PosMod.z = PosMod.z + 0.5
			end
		end
		...
		PosMod.z = PosMod.z - self:scaleMovement(tan1 * -0.3, mod2) -- up/down
		
		-- apply viewmodel tilt when moving and not aiming based on velocity dot product relative to aim direction
		local norm = math.Clamp(vel:GetNormal():DotProduct(self.Owner:EyeAngles():Forward()), 0, 4)
		
		if self.dt.State ~= CW_AIMING then
			TargetPos[2] = TargetPos[2] - mul * 1 * norm
			TargetPos[3] = TargetPos[3] - mul * 0.6 * norm
		end
	end
